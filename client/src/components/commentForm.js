import React, { useState } from "react";
import { Link } from "react-router-dom";
import {useUser} from "../auth/useUser"





export function CommentForm ({blogName, setBlogInfo }){
    
     const [,setUsername] = useState('');
     const [commentText, setCommentText] = useState('');
     
     const user = useUser();
     const { username, given_name,name } = user || '';

     const addComment = async () => {
          const response = await fetch(`http://localhost:8080/api/blogs/${blogName}/add-comment`, {
            method: 'post',
            body: JSON.stringify({ username,name,given_name,text: commentText }),
            headers: { 
                'Content-Type': 'application/json',
            }
        });
        const body = await response.json();
        setBlogInfo(body);
        setUsername(username||given_name||name);
        setCommentText('');
    };

    if (user) {
        return (
            <div id='commentForm'>
                <h3>Comment as <span><Link to={'/user/profile'}>{ username||given_name||name}</Link></span></h3>
                <label>
                    <textarea rows='4' cols='50' value={commentText} onChange={(event) => setCommentText(event.target.value)} />
                </label>
                <button id='submit' disabled={!commentText} onClick={() => addComment()}>Comment</button>
            </div>
        )
    } return (
        <div  id='other'>
            <Link to={'/signin'}><p> Sign in or register to comment</p></Link>
        </div>
    );
};
