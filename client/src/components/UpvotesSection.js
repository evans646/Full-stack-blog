...
const UpvotesSection = ({ articleName, upvotes, setArticleInfo,hasUpvoted}) => {
  ...
  <Container id='upvotes-section'>
   {!user ? (<button onClick={() => history.push('/login')}> Upvote</button>) : (<button onClick={() => upvoteArticle()}>Upvote</button>) && !hasUpvoted ? ( <button onClick={() =>  upvoteArticle()}>Upvote</button>):(<button onClick={() => downvoteArticle()}>Downvote</button>)}
